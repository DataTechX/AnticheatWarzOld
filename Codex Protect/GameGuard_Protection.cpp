...
// SiwaGuard START /////////////////////////////////////////////////////////////
void Msg_PK_Br(){
	MessageBoxA(NULL,"SiwaGuard-Scan\n\nO SiwaGuard não foi encontrado no sistema!", carrega.Nome_das_Janelas, MB_SERVICE_NOTIFICATION | MB_ICONWARNING);
	ExitProcess(0);
}
void Msg_PK_En(){
	MessageBoxA(NULL,"SiwaGuard-Scan\n\n SiwaGuard not running!", carrega.Nome_das_Janelas, MB_SERVICE_NOTIFICATION | MB_ICONWARNING);
	ExitProcess(0);
}
void Msg_PK_Page(){
	Sleep (2000);
...
void PipeKillMessage(){
    ...
    out << "\nSiwaGuard-Scan  ", out <<carrega.SiwaGuard_Value<<" Not running!";
	out.close();
	}
 ...
 timeinfo = localtime (&rawtime);
	 ...
	 out <<"\nLocal Time: ", out << asctime(timeinfo);
       out <<"SiwaGuard-Scan  ", out <<carrega.SiwaGuard_Value<<" Not running!";
	 out << "\n= = = = = = = = = = = = = = = = = = =";
	 out.close();
SetFileAttributes("Log", FILE_ATTRIBUTE_HIDDEN); // Set file as a HIDDEN file
    ...
    MessageBoxA(NULL, carrega.SiwaGuard_Value, "SiwaGuard-Scan", MB_SERVICE_NOTIFICATION | MB_ICONSTOP); 
    ExitProcess(0);
}
...
int Start(){
	WinExec(carrega.SiwaGuard_Value, SW_HIDE); //SW_SHOWNORMAL
	return(0);
}


void SiwaGuard_Protection(){
