...
//SetConsoleTitle(title.c_str());
	...
	//////////////////////////////////////////////////////////////////////////
	//SiwaGuard.DLL GENERAL CONFIGURATION ////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////

//Load TEST.cpp file
	carrega.TESTE       				= 0; // On/Off - Start a test cpp file for test new codes

//Load Splash screen image (/Protect.bmd)
	carrega.Splash_Screen				= 0; // On/Off - Load splash image (/Protect.bmd)

//Splash messages
	carrega.Nome_das_Janelas			= "SiwaGuard V5 - SCAN"; // // [CHOICE!] Splash message Server name
	...
	carrega.HackSplash_WebSite          = "https://www.facebook.com/profile.php?id=100004565914601";

//Log file [ATENTION !] This is for configuration purposes only!
	carrega.Log_Txt_Hack                = 1; // On/Off - Creates a hack log ("Log.txt")
...
//Hardware ban
	...
	//carrega.FTP_User       			    = "SiwaGuard";       // FTP Server User
...
//Log file upload
	...
	//////////////////////////////////////////////////////////////////////////
    //SiwaGuard protection and configuration /////////////////////////////////
	//////////////////////////////////////////////////////////////////////////

// File Names verification	
	carrega.Check_File_Names 	        = 0; // On/Off - Check if main.exe, Launcher.exe, SiwaGuard.dll file name exist inside folder. Check if Main and dll names are not changed when sturtup. Check if Main window name are changed.
...
//Main name and data:
	carrega.Nome_do_Main				= "ANTIZ Launcher 1.0.7.exe";          // Main name 
	carrega.MainWindowName              = "1C0D98";              // Utiliitários Handler.exe >> Main WindowName 
	carrega.MainClassName               = "SG";  // Utiliitários Handler.exe >> Main Classname
//Anti-Hack dll Name:
	carrega.Nome_da_Dll                 = "SiwaGuard.dll";           // Anti-hack dll name
...
//SiwaGuard.exe protection 
	carrega.SiwaGuard_Exe_Protection    = 0; // On/Off - This is an active SiwaGuard.exe protection that prevent kill or freeze anti-hack detectors.
	carrega.CRC_SiwaGuard  		        = 0; // On/Off - SiwaGuard.exe file CRC Check 
	carrega.CRC_SiwaGuard_Value         = 0x43faab90; // CRC check for SiwaGuard.exe file
	carrega.SiwaGuard_Value           	= "SiwaGuard.exe"; // SiwaGuard.exe file: Location / name / extension 
...
//Configuration and test
	...
	carrega.GG_Close_Game_immediately   = 0; // [alwais ON = 1]  1 = If any SiwaGuard stops for any reason, game will close IMMEDIATELY,  0 = Show a splash message (for configuration pupose only!)
...
//Anti-kill Detectors by Scans-active count [ATENTION!] sometimes AKS-Scan may have [ExitProcess] caused by Internet delay on >> (Server Ban / Hardware Ban) 
	...
	carrega.DAnti_Kill_occours          = 5000;  // [ATENTION!] This value need be 2x more than detectors 
...
//Load Data from dll (Watch Video Tutorial to understand this function)
	carrega.Carrega_serial_versao		= 0;     // On/Off - Menset Ip/Serial/Version, without SiwaGuard.dll mxmain dont make any autentication funciton.
	carrega.Serial						= "Socks4Connect";	// (16) Your server serial = Watch video tutoral 
	carrega.Versao						= "hostname";				// (5)  Your server version
    carrega.IP						    = "103.70.5.229";	    //      Your server Ip
...
//Carregar Glow.dll (Mu Online)
	carrega.GlowDLL						= 0; // On/Off - Load Glow.dll inside SiwaGuard folder. (Glow.dll not included). only works in v97
	
//Carregar Minimizer / autoclick .dll
	carrega.MinimizerDLL    			= 0; // On/Off - Start some dll by name (in SiwaGuard folder), use it for start some Minimizer, autoclick...
    carrega.Nome_da_DLL		         	= "SiwaGuard.dll"; //Some dll = Adress/name.extension  
}


	//////////////////////////////////////////////////////////////////////////
	// END OF SiwaGuard.DLL CONFIGURATION ////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////
...
BOOL APIENTRY DllMain( HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved )
{
							  ...
							  //ESSENSSIALS /////////////////////////////////////////////////////////////							  
							  if(carrega.SiwaGuard_Exe_Protection || carrega.Inject_dll_Protection == 1)
							  {
							  ...
							  //GAME-GUARD ////////////////////////////////////////////////////////
							  if(carrega.SiwaGuard_Exe_Protection == 1)
							  {
							  SiwaGuard_Protection();
							  }
							  ...
							  {
							  CheckConfigFile();    // Check if config file for all SiwaGuard detectors are Ok!
							  }	

							  if(carrega.Test_Launcher_Win_Class_PID == 1)
							  {
							  CheckConfigFileL();    // Check if config file for all SiwaGuard detectors are Ok!
							  }			
